...
func (s *TestSuite) TestUploadTarball() {
	catalogClient := methods.CatalogClient{
		...
		ProjectID:            s.catalogClient.ProjectID,
	}

	res, err := catalogClient.UploadTarball(types.WordpressTarballPathName)
	...
	// Cleanup
	s.NoError(catalogClient.DeleteDeploymentPackage(types.WordpressName, types.WordpressVersion, true), "Expected to delete deployment package after upload")
	s.NoError(catalogClient.DeleteApplication(types.WordpressName, types.WordpressVersion, true), "Expected to delete application after upload")
	s.NoError(catalogClient.DeleteRegistry(types.WordpressRegistryName, true), "Expected to delete registry after upload")
}
