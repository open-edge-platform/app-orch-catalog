...
The object model shown in the [architecture](architecture.md) is implemented by two models:

- an [ENT schema](../internal/ent/schema)
- a [protobuf definition](../api/catalog/v3/resources.proto)

The application catalog itself is started from [main](../cmd/application-catalog/application-catalog.go),
and runs a gRPC server on Port 8080 and connects to a Postgres database on the backend on Port 5432.

A second executable provides a `REST` front end and is started with [rest-proxy](../cmd/rest-proxy/rest-proxy.go), and
...
The application catalog uses the ORM [entgo.io], which supports both automatic migration and versioned migration.

While automatic migration is easy, it is also limited to relatively simple schema changes. In order to provide
more robust migration capabilities, the application catalog is switching to using the versioned migration approach, which
is facilitated by the entgo ORM and [Atlas] - an open-source database schema management system.
...
change records that track how the schema changes and from that generate versioned migration files.
...
When the developer makes changes to the [schema](../internal/ent/schema) package, they will need to run the following
...
This step will now also result in the creation of files under the `internal/ent/migrate/migrations`
...
to the new/future state. These commands, along with hashes, will be stored in the `internal/ent/migrate/migrations` directory,
...
The output of this command may include warnings about potential issues, which may need to be addressed by
additional manually written code. If schema changes are made deliberately, this should not be necessary, but sometimes
it may not be entirely avoidable.

At this point, the developer or DB admin has all the collateral to apply (or execute) the migration.

## Applying Migrations

_Note: This section hasn't yet been vetted as the application catalog service presently initializes the Postgres
...
scripts on an offline copy of the production system to make sure there are no unexpected issues, which could
...
_Note: The `--url` option specified above is just an example. Obviously, this would differ in a production environment._

## More Documentation

More detailed documentation is provided on the [entgo.io versioned migrations] page.
The above page provides merely a summary and a distillation of concrete steps to be executed in the context of
this specific project and database drivers.
