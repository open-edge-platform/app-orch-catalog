...
type ArtifactQuery struct {
	...
	withCaIconFk      *DeploymentPackageQuery
	withCaThumbnailFk *DeploymentPackageQuery
...
// QueryCaIconFk chains the current query on the "ca_icon_fk" edge.
func (aq *ArtifactQuery) QueryCaIconFk() *DeploymentPackageQuery {
...
// QueryCaThumbnailFk chains the current query on the "ca_thumbnail_fk" edge.
func (aq *ArtifactQuery) QueryCaThumbnailFk() *DeploymentPackageQuery {
	query := (&DeploymentPackageClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (aq *ArtifactQuery) FirstID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (aq *ArtifactQuery) FirstIDX(ctx context.Context) uint64 {
...
func (aq *ArtifactQuery) OnlyID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (aq *ArtifactQuery) OnlyIDX(ctx context.Context) uint64 {
...
func (aq *ArtifactQuery) IDs(ctx context.Context) (ids []uint64, err error) {
...
func (aq *ArtifactQuery) IDsX(ctx context.Context) []uint64 {
	ids, err := aq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (aq *ArtifactQuery) Clone() *ArtifactQuery {
	...
	return &ArtifactQuery{
		...
		withCaIconFk:      aq.withCaIconFk.Clone(),
		withCaThumbnailFk: aq.withCaThumbnailFk.Clone(),
...
// WithCaIconFk tells the query-builder to eager-load the nodes that are connected to
// the "ca_icon_fk" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *ArtifactQuery) WithCaIconFk(opts ...func(*DeploymentPackageQuery)) *ArtifactQuery {
	query := (&DeploymentPackageClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withCaIconFk = query
	return aq
}

// WithCaThumbnailFk tells the query-builder to eager-load the nodes that are connected to
// the "ca_thumbnail_fk" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *ArtifactQuery) WithCaThumbnailFk(opts ...func(*DeploymentPackageQuery)) *ArtifactQuery {
	query := (&DeploymentPackageClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withCaThumbnailFk = query
	return aq
}
...
func (aq *ArtifactQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Artifact, error) {
	...
	if query := aq.withCaIconFk; query != nil {
		if err := aq.loadCaIconFk(ctx, query, nodes,
			func(n *Artifact) { n.Edges.CaIconFk = []*DeploymentPackage{} },
			func(n *Artifact, e *DeploymentPackage) { n.Edges.CaIconFk = append(n.Edges.CaIconFk, e) }); err != nil {
			return nil, err
		}
	}
	if query := aq.withCaThumbnailFk; query != nil {
		if err := aq.loadCaThumbnailFk(ctx, query, nodes,
			func(n *Artifact) { n.Edges.CaThumbnailFk = []*DeploymentPackage{} },
			func(n *Artifact, e *DeploymentPackage) { n.Edges.CaThumbnailFk = append(n.Edges.CaThumbnailFk, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *ArtifactQuery) loadCaIconFk(ctx context.Context, query *DeploymentPackageQuery, nodes []*Artifact, init func(*Artifact), assign func(*Artifact, *DeploymentPackage)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uint64]*Artifact)
	nids := make(map[uint64]map[*Artifact]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(artifact.CaIconFkTable)
		s.Join(joinT).On(s.C(deploymentpackage.FieldID), joinT.C(artifact.CaIconFkPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(artifact.CaIconFkPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(artifact.CaIconFkPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "ca_icon_fk" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (aq *ArtifactQuery) loadCaThumbnailFk(ctx context.Context, query *DeploymentPackageQuery, nodes []*Artifact, init func(*Artifact), assign func(*Artifact, *DeploymentPackage)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uint64]*Artifact)
	nids := make(map[uint64]map[*Artifact]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(artifact.CaThumbnailFkTable)
		s.Join(joinT).On(s.C(deploymentpackage.FieldID), joinT.C(artifact.CaThumbnailFkPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(artifact.CaThumbnailFkPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(artifact.CaThumbnailFkPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "ca_thumbnail_fk" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (aq *ArtifactQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(artifact.Table, artifact.Columns, sqlgraph.NewFieldSpec(artifact.FieldID, field.TypeUint64))
	_spec.From = aq.sql
