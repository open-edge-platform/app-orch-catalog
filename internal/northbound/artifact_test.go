...
func (s *NorthBoundTestSuite) TestCreateArtifactInvalidName() {
	...
	s.ErrorIs(err, status.Errorf(codes.InvalidArgument,
		"artifact Bad name invalid: validation error: artifact.name: does not match regex pattern `^[a-z0-9][a-z0-9-]{0,38}[a-z0-9]{0,1}$`"))
}
...
func (s *NorthBoundTestSuite) TestCreateArtifactMimeType() {
	...
	s.ErrorIs(err, status.Errorf(codes.InvalidArgument,
		"artifact test-artifact invalid: validation error: artifact.mime_type: does not match regex pattern `^(text/plain)$|^(application/json)$|^(application/yaml)$|^(image/png)$|^(image/jpeg)$`"))
	...
	s.ErrorIs(err, status.Errorf(codes.InvalidArgument,
		"artifact test-artifact invalid: validation errors:\n - artifact.mime_type: must be at least 1 characters\n - artifact.mime_type: does not match regex pattern `^(text/plain)$|^(application/json)$|^(application/yaml)$|^(image/png)$|^(image/jpeg)$`"))
}
...
func (s *NorthBoundTestSuite) TestCreateArtifactDataPlain() {
	...
	s.ErrorIs(err, status.Errorf(codes.InvalidArgument,
		"artifact test-artifact invalid: validation error: artifact.artifact: must be at least 4 bytes"))
...
new line`)
	...
	f.Fuzz(func(t *testing.T, name string, displayName string) {
		...
		nameSpacesPatternRE, _ := regexp.Compile(`(?s)validation errors?:.*artifact\.name: does not match regex pattern.*`)
		nameLenRE, _ := regexp.Compile(`(?s)validation errors?:.*artifact\.name: must be at most.*characters.*`)
		...
		displayNameLenRE, _ := regexp.Compile(`(?s)validation errors?:.*artifact\.display_name: must be at most.*characters.*`)
		displayNamePatternRE, _ := regexp.Compile(`(?s)validation errors?:.*artifact\.display_name: does not match regex pattern.*`)
		valueLenRE, _ := regexp.Compile(`(?s)validation errors?:.*artifact\.artifact: must be at least.*bytes.*`)
