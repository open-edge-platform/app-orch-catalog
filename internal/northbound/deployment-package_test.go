...
func (s *NorthBoundTestSuite) TestDeploymentPackageEventsWithReplay() {
	s.T().Skip()
	ctx, cancel := context.WithCancel(s.ProjectID(footen))
	stream, err := s.client.WatchDeploymentPackages(ctx, &catalogv3.WatchDeploymentPackagesRequest{})
	s.NoError(err)

	existing := "ca-gigi ca-fifi"
	for i := 0; i < 3; i++ {
		resp, err := stream.Recv()
		s.NoError(err)
		s.Equal(ReplayedEvent, EventType(resp.Event.Type))
		s.True(strings.Contains(existing, resp.DeploymentPackage.Name), "unexpected: %s", resp.DeploymentPackage.Name)
	}
...
type catalogServiceWatchDPsServer struct {
	testServerStream
	sendError bool
}

func (c *catalogServiceWatchDPsServer) Send(*catalogv3.WatchDeploymentPackagesResponse) error {
	if c.sendError {
...
func (s *NorthBoundDBErrTestSuite) TestWatchDPsDatabaseErrors() {
	...
	server := &catalogServiceWatchDPsServer{sendError: false}
	req := &catalogv3.WatchDeploymentPackagesRequest{}

	// Test unable to start a transaction
	err := s.server.WatchDeploymentPackages(req, server)
	...
	wg.Add(1)
	go func() {
		err = s.server.watchDeploymentPackageEvents(server, ch)
		wg.Done()
	}()
	ch <- nil
	wg.Wait()
...
func (s *NorthBoundDBErrTestSuite) TestCreateDeploymentPackageDatabaseErrors() {
	s.T().Skip("FIXME: Hard to troubleshoot")
	...
	var resp *catalogv3.CreateDeploymentPackageResponse
	var err error
	...
	testCases := []testCase{
		{
			description: "unable to start a transaction",
			step:        func() {},
		},
		{
			description: "publisher query database error",
			step: func() {
				deploymentPackageRequest.DeploymentPackage.Profiles = []*catalogv3.DeploymentProfile{
					{
						Name: "p1",
					},
				}
				deploymentPackageRequest.DeploymentPackage.DefaultProfileName = "p"
				s.mock.ExpectBegin()
			},
		},
		{
			description: "deployment package save database error",
			step: func() {
				s.addMockedQueryRowsWithResult(1, 1)
			},
		},
		{
			description: "create application references database error",
			step: func() {
		...
		{
			description: "create application dependencies app query database error",
			step: func() {
		...
		{
			description: "create deployment database query database error",
			step: func() {
				deploymentPackageRequest.DeploymentPackage.Profiles = []*catalogv3.DeploymentProfile{
					{
						Name: "p1",
		...
		{
			description: "application namespaces app query database error",
			step: func() {
				s.addMockedQueryRowsWithResult(1, 1)
			},
		},
		{
			description: "update default profile database error",
			step: func() {
				s.mock.ExpectExec("INSERT INTO .*").
					WillReturnResult(sqlmock.NewResult(1, 1))
			},
		...
		{
			description: "create extension database error",
			step: func() {
				deploymentPackageRequest.DeploymentPackage.Extensions = []*catalogv3.APIExtension{
					{
						Name:    "e1",
						Version: "0.11.22",
					},
		...
		{
			description: "create artifact references database error",
			step: func() {
				deploymentPackageRequest.DeploymentPackage.Artifacts = []*catalogv3.ArtifactReference{{Name: "a1"}}
				s.mock.ExpectExec("INSERT INTO .*").
					WillReturnResult(sqlmock.NewResult(1, 1))
			},
		},
		{
			description: "transaction commit error",
			step: func() {
				s.addMockedQueryRowsWithResult(1, 1)
				s.mock.ExpectExec("INSERT INTO .*").
					WillReturnResult(sqlmock.NewResult(1, 1))
	...
	for stepNumber, test := range testCases {
...
func (s *NorthBoundDBErrTestSuite) TestDeploymentPackageNamespacesDBError() {
	s.T().Skip("FIXME: Hard to troubleshoot")
	deploymentPackageRequest := &catalogv3.CreateDeploymentPackageRequest{
		DeploymentPackage: &catalogv3.DeploymentPackage{
			Name:               "p",
			...
			DefaultNamespaces:  map[string]string{"foo": "foo"},
		},
	}
	s.mock.ExpectBegin()
	s.addMockedQueryRowsWithResult(1, 1)
	...
	s.mock.ExpectExec("UPDATE .*").WillReturnResult(sqlmock.NewResult(1, 1))
	s.mock.ExpectExec("DELETE FROM .*").WillReturnResult(sqlmock.NewResult(1, 1))
	s.addMockedQueryRowsWithResult(2, 1)
	rows := sqlmock.NewRows([]string{"id", "name", "display_name", "display_name_lc", "description", "create_time", "update_time", "version", "chart_name", "chart_version"}).
		AddRow(12, "foo", "app", "app", "app", time.Now(), time.Now(), "1.2.3", "chart", "0.0.1")
	s.mock.ExpectQuery("SELECT .*").WillReturnRows(rows)
	resp, err := s.server.CreateDeploymentPackage(s.ctx, deploymentPackageRequest)
	s.validateDBError(err, resp)
}

func (s *NorthBoundDBErrTestSuite) TestDeploymentPackageAppReferenceDBError() {
	s.T().Skip("FIXME: Hard to troubleshoot")
	// Test application reference app query database error
	s.mock.ExpectBegin()
	s.addMockedQueryRowsWithResult(1, 1)
	s.mock.ExpectExec("INSERT INTO .*").WillReturnResult(sqlmock.NewResult(1, 1))
	resp, err := s.server.CreateDeploymentPackage(s.ctx, &catalogv3.CreateDeploymentPackageRequest{
		DeploymentPackage: &catalogv3.DeploymentPackage{
			Name: "bar", Version: "v0.3.0",
			ApplicationReferences: []*catalogv3.ApplicationReference{
				{Name: "non-existent-application", Version: "v0.2.0"},
...
func (s *NorthBoundDBErrTestSuite) TestListDeploymentPackageDatabaseErrors() {
	s.T().Skip("FIXME: Hard to troubleshoot")
	deploymentPackageRequest := &catalogv3.ListDeploymentPackagesRequest{}
	var resp *catalogv3.ListDeploymentPackagesResponse
	var err error
	...
	testCases := []testCase{
		...
		{
			description: "deployment package query database error",
			step: func() {
				s.addMockedQueryRowsWithResult(1, 1)
			},
		},
		{
			description: "deployment package extract publisher query database error",
			step: func() {
				s.addMockedQueryRowsWithResult(1, 1)
			},
		},
		{
			description: "deployment package extract application references query database error - 1",
			step: func() {
				s.addMockedQueryRowsWithResult(1, 1)
			},
		},
		{
			description: "deployment package extract profiles query database error - 2",
			step: func() {
				s.addMockedQueryRowsWithResult(1, 1)
			},
		},
		{
			description: "deployment package extract profiles query database error - 1",
			step: func() {
				s.addMockedQueryRowsWithResult(1, 1)
			},
		},
		...
		{
			description: "deployment package extract application dependencies query database error - 1",
			step: func() {
				s.addMockedQueryRowsWithResult(1, 1)
			},
		},
		{
			description: "deployment package extract application dependencies query database error - 2",
			step: func() {
				s.addMockedQueryRowsWithResult(1, 1)
			},
		},
		{
			description: "deployment package extract application dependencies query database error - 3",
			step: func() {
				s.addMockedQueryRowsWithResult(1, 1)
			},
		},
		{
			description: "deployment package extract application namespaces query database error - 1",
			step: func() {
				s.addMockedQueryRowsWithResult(1, 1)
			},
		},
		{
			description: "deployment package extract application namespaces query database error - 2",
			step: func() {
				s.addMockedQueryRowsWithResult(1, 1)
			},
		},
		{
			description: "deployment package extract application namespaces query database error - 3",
			step: func() {
				s.addMockedQueryRowsWithResult(1, 1)
			},
		},
		{
			description: "deployment package extract extensions query database error - 1",
			step: func() {
				s.addMockedQueryRowsWithResult(1, 1)
			},
		},
		{
			description: "deployment package extract extensions query database error - 2",
			step: func() {
				s.addMockedQueryRowsWithResult(1, 1)
			},
		},
		{
			description: "deployment package extract extensions query database error - 3",
			step: func() {
				s.addMockedQueryRowsWithResult(1, 1)
			},
		},
		{
			description: "deployment package extract artifacts query database error",
			step: func() {
				s.addMockedQueryRowsWithResult(1, 1)
			},
		},
		{
			description: "deployment package default profile query database error - 1",
			step: func() {
				s.addMockedQueryRowsWithResult(1, 1)
			},
		},
		{
			description: "deployment package default profile query database error - 2",
			step: func() {
				s.addMockedQueryRowsWithResult(1, 1)
			},
		},
		{
			description: "deployment package default profile query database error - 3",
			step: func() {
				s.addMockedQueryRowsWithResult(1, 1)
			},
		},
	...
	for stepNumber, test := range testCases {
		s.Run(test.description, func() {
			...
			resp, err = s.server.ListDeploymentPackages(s.ctx, deploymentPackageRequest)
			s.validateDBError(err, resp)
		})
	}
}

func (s *NorthBoundDBErrTestSuite) TestGetDeploymentPackageDatabaseErrors() {
	s.T().Skip("FIXME: Hard to troubleshoot")
	deploymentPackageRequest := &catalogv3.GetDeploymentPackageRequest{DeploymentPackageName: "p", Version: "9"}
	var resp *catalogv3.GetDeploymentPackageResponse
	var err error
	...
	testCases := []testCase{
		...
		{
			description: "deployment package extract query database error",
			step: func() {
				s.addMockedQueryRowsWithResult(1, 1)
			},
		},
		{
			description: "transaction commit database error",
			step: func() {
				s.addMockedQueryRowsWithResult(17, 1)
			},
		},
	}
	...
	for stepNumber, test := range testCases {
		s.Run(test.description, func() {
			...
			resp, err = s.server.GetDeploymentPackage(s.ctx, deploymentPackageRequest)
			s.validateDBError(err, resp)
		})
	}
}
...
func (s *NorthBoundDBErrTestSuite) TestGetDeploymentPackageVersionsDatabaseErrors() {
	s.T().Skip("FIXME: Hard to troubleshoot")
	deploymentPackageRequest := &catalogv3.GetDeploymentPackageVersionsRequest{DeploymentPackageName: "p"}
	var resp *catalogv3.GetDeploymentPackageVersionsResponse
	var err error
	...
	testCases := []testCase{
		...
		{
			description: "deployment package extraction database error",
			step: func() {
				s.addMockedQueryRowsWithResult(1, 1)
			},
		},
	...
	for stepNumber, test := range testCases {
		s.Run(test.description, func() {
			...
			resp, err = s.server.GetDeploymentPackageVersions(s.ctx, deploymentPackageRequest)
			s.validateDBError(err, resp)
		})
	}
}
...
func (s *NorthBoundDBErrTestSuite) TestUpdateDeploymentPackageDatabaseErrors() {
	s.T().Skip("FIXME: Hard to troubleshoot")
	deploymentPackageRequest := &catalogv3.UpdateDeploymentPackageRequest{
		DeploymentPackageName: "p",
		Version:               "1234",
		DeploymentPackage: &catalogv3.DeploymentPackage{
			Name:               "p",
			Version:            "1234",
	...
	testCases := []testCase{
		...
		{
			description: "app reference query database error",
			step: func() {
				s.addMockedQueryRowsWithResult(1, 1)
			},
		},
		{
			description: "app references changed query database error",
			step: func() {
				s.addMockedQueryRowsWithResult(1, 1)
			},
		},
		{
			description: "compute package changes DB error",
			step: func() {
				s.addMockedQueryRowsWithResult(1, 1)
			},
		},
		{
			description: "checkDeploymentPackageNotDeployed database error",
			step: func() {
				s.addMockedQueryRowsWithResult(17, 1)
			},
		},
		{
			description: "deployment package uniqueness database error",
			step: func() {
				s.addMockedQueryRowsWithResult(1, 1)
			},
		},
		{
			...
			step: func() {
				s.addMockedQueryRowsWithResult(1, 0)
			},
		},
		{
			description: "create application references",
			step: func() {
				s.mock.ExpectExec("UPDATE .*").WillReturnResult(sqlmock.NewResult(1, 1))
			},
		},
		{
			description: "update app dependencies database error",
			step: func() {
				s.mock.ExpectExec("UPDATE .*").WillReturnResult(sqlmock.NewResult(1, 1))
				s.mock.ExpectExec("DELETE FROM .*").WillReturnResult(sqlmock.NewResult(1, 1))
				s.addMockedQueryRowsWithResult(1, 0)
		...
		{
			description: "update app name spaces database error",
			step: func() {
				s.mock.ExpectExec("DELETE FROM .*").WillReturnResult(sqlmock.NewResult(1, 1))
				s.addMockedQueryRowsWithResult(1, 0)
			},
		...
		{
			description: "update deployment profiles database error",
			step: func() {
				s.mock.ExpectExec("DELETE FROM .*").WillReturnResult(sqlmock.NewResult(1, 1))
				s.addMockedQueryRowsWithResult(1, 0)
			},
		...
		{
			description: "default deployment profile query",
			step: func() {
				s.addMockedQueryRowsWithResult(1, 0)
			},
		},
		{
			description: "query deployment profile database error",
			step: func() {
				s.mock.ExpectExec("DELETE FROM .*").WillReturnResult(sqlmock.NewResult(1, 1))
			},
		},
		{
			description: "update deployment profile database error",
			step: func() {
				s.addMockedQueryRowsWithResult(1, 0)
			},
		},
		{
			description: "update extensions database error",
			step: func() {
				s.mock.ExpectExec("UPDATE .*").WillReturnResult(sqlmock.NewResult(1, 1))
			},
		},
		{
			description: "update artifacts database error",
			step: func() {
				s.mock.ExpectExec("DELETE FROM .*").WillReturnResult(sqlmock.NewResult(1, 1))
			},
		},
	...
	for stepNumber, test := range testCases {
		s.Run(test.description, func() {
			...
			resp, err = s.server.UpdateDeploymentPackage(s.ctx, deploymentPackageRequest)
			s.validateDBError(err, resp)
		})
	}
}
